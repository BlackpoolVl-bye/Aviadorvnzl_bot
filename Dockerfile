...
# Instalar dependencias con proxy (opcional)
RUN apt-get update && \
    apt-get install -y tor && \
    pip install --proxy=http://tupoxy:puerto pysocks websocket-client

COPY . .
RUN pip Instalar pysocks
...
# Variables de entorno (pueden sobrescribirse al ejecutar)
